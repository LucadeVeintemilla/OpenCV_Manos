...
class detectormanos():
    def __init__(self, mode=False, maxManos=2, modelComplexity=1, Confdeteccion=0.5, Confsegui=0.5):
        ...
        self.hands = self.mpmanos.Hands(self.mode, self.maxManos, self.modelComplex, self.Confdeteccion, self.Confsegui)
    ...
    def encontrarmanos(self, frame, dibujar=True):
        ...
        self.resultados = self.hands.process(imgcolor)
    ...
    def encontrarposicion(self, frame, ManoNum=0, dibujar=True):
        ...
        if self.resultados.multi_hand_landmarks:
            ...
            for id, lm in enumerate(miMano.landmark):
                ...
                cx, cy = int(lm.x * alto), int(lm.y * alto)
                xlista.append(cx)
                ylista.append(cy)
